...
describe('RentNft', function () {
  ...
  context('Renting', async function () {
    ...
    it('rents ok - one eth', async () => {
      ...
      const nftContractType = [NftContractType.ERC721];
      const tokenId = [1];
      const lendingId = [1];
    ...
    it('rents ok - one eth & one erc20', async () => {
      ...
      const nftContractTypes = Array(2).fill(NftContractType.ERC721);
      const lendingId = [1, 2];
    ...
    it('does not rent - rent duration is zero', async () => {
      ...
      const nftAddress = [ERC721.address];
      const nftContractTypes = Array(1).fill(NftContractType.ERC721);
      const tokenId = [1];
      const lendingId = [1];
    ...
    it('does not rent - you are lender', async () => {
      ...
      await lender.renft.lend(
        Array(tokenIds.length).fill(ERC721.address),
        Array(2).fill(NftContractType.ERC721),
        tokenIds,
        [maxRentDurations],
        _dailyRentPrices,
        _nftPrices,
      ...
      await expect(
        lender.renft.rent(
          nftAddress,
          Array(1).fill(NftContractType.ERC721),
          tokenId,
          lendingId,
          rentDuration,
          {
  ...
  context('Collateral Claiming', async function () {
    ...
    it('claims collateral ok - one eth one erc20', async () => {
      ...
      await renter.renft.rent(
        _nft,
        [NftContractType.ERC721, NftContractType.ERC721],
        _tokenId,
        _id,
        _rentDuration,
        {
