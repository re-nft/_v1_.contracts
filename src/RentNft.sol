...
contract RentNft is ReentrancyGuard, Ownable, ERC721Holder {
  ...
  function rentOne(
    ...
    uint256 rentPrice = _rentDuration *
      _unpackPrice(bytes4(item.lending.dailyRentPrice));
  ...
  function _takeFee(uint256 nftPrice, uint256 rent, IERC20 paymentToken) internal returns (uint256 fee) {
    fee = nftPrice * nftPriceFee + rent * rentFee;
    paymentToken.safeTransfer(beneficiary, fee);
    fee /= 2;
  }

  function _distributePayments(IERC721 _nftAddress, uint256 _tokenId, uint256 _id, uint256 _secondsSinceRentStart) internal {
    LendingRenting storage item = lendingRenting[keccak256(
      abi.encodePacked(address(_nftAddress), _tokenId, _id)
    )];
    IERC20 paymentToken = resolver.getPaymentToken(
    ...
    paymentToken.safeTransfer(
      item.renting.renterAddress,
      sendRenterBackAmt - halfFee
    );
  ...
  function _distributeClaimPayment(IERC721 _nftAddress, uint256 _tokenId, uint256 _id) internal {
    LendingRenting storage item = lendingRenting[keccak256(
      abi.encodePacked(address(_nftAddress), _tokenId, _id)
    )];
    IERC20 paymentToken = resolver.getPaymentToken(
  ...
  function returnOne(
    ...
    uint16 durationInDays = uint16(secondsSinceRentStart /
      SECONDS_IN_A_DAY);
